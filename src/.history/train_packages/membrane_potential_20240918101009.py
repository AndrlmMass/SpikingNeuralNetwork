...
def update_membrane_potential_conduct(
    ...
    # Update spike indices
    S_j_inh = np.expand_dims(S[N_input_neurons:], axis=1)
    S_i = np.expand_dims(S[-N_inhib_neurons:], axis=1)  # postsynaptic spikes

    # Update weight indices
    w_ij_exc = W_static[:-N_inhib_neurons]  # excitatory weights
    w_ij_inh = W_static[-N_inhib_neurons:]  # inhibitory weights
