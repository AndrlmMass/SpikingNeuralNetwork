...
def normalize_weights_per_column(
    ...
    scale = np.where(current_sums > 1e-8, np.minimum(1.0, initial_sums / current_sums), 1.0)
        ...
        for i in range(N_x, n_neurons):
            if spikes[i] == 1:
                ...
                for j in pre_indices:
                    ...
                    delta_w = learning_rate_exc * first_trm * base
                    weights[j, i] += delta_w
                    list_x_pre += x_pre_exc
                    first_term += first_trm
                    delta_w_sum += delta_w
                    count += 1
        return weights, list_x_pre/(count+1e-5), first_term/(count+1e-5), delta_w_sum/(count+1e-5)
    else:
