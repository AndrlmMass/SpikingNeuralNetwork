...
class TrainTracker:
    ...
    def track_synapse(self, m_x_pre, m_first_term, m_delta_w, m_ltp, m_ltd, x_tar_se, x_tar_ee):
        '''
        Computes mean scores for tracking periodically when called.
        '''
    ...
    def to_dict(self) -> Optional[dict]:
        ...
        d = {
            ...
            "mean_ltp": self.ltp_sum / nx,
            "mean_ltd": self.ltd_sum / nx,
            "ltp_ltd_ratio": self.ltp_sum / (self.ltd_sum + 1e-9),
    ...
    def print(
        ...
        if d is None:
            return
        # label the block by pass so train vs val/test diagnostics are unambiguous
        if training_mode is not None:
            print(f"========== Diagnostics [mode: {training_mode}] ==========")
        # compute spike means
        ...
        if track_stats:
            ...
            # Live x_tar actually used by the STDP rule this pass — independent of
            # track_weights. In 'val'/'test' these sit at their t=0 init (~0) because
            # weight updates (and thus update_x_tar) never run; only in 'train' should
            # they be nonzero. If they're ~0 during 'train', the rule has no LTD term
            # (first_trm = trace - x_tar collapses to trace >= 0 -> potentiation-only).
            if x_tar_se is not None:
                print(f"Live x_tar SE (used):     {float(x_tar_se):.5f}  [x_tar_mode: {x_tar_mode}]")
                print(f"Live x_tar EE (used):     {float(x_tar_ee):.5f}")
        ...
        if track_weights:
            ...
            print(f"LTP / LTD mag:            {d['mean_ltp']:.6f} / {d['mean_ltd']:.6f}  (ratio {d['ltp_ltd_ratio']:.3f})")
    ...
    def _print_diagnostics(self, weights, spikes, spike_trace):
        ...
        # weight-block magnitudes (over nonzero structural synapses) and per-layer
        # spike rates — stored here so they become a time series via to_dict/_diag.
        # Mirrors what print() reports but persists it for the stats figure.
        nz = lambda block: block[block != 0]
        W_ei = weights[self.st : self.ex, self.ex : self.ih]
        W_ie = weights[self.ex : self.ih, self.st : self.ex]
        _se, _ee, _ei, _ie = nz(W_se), nz(W_ee), nz(W_ei), nz(W_ie)
        w_se_mean = float(_se.mean()) if _se.size else 0.0
        w_se_std = float(_se.std()) if _se.size else 0.0
        w_ee_mean = float(_ee.mean()) if _ee.size else 0.0
        w_ee_std = float(_ee.std()) if _ee.size else 0.0
        w_ei_mean = float(_ei.mean()) if _ei.size else 0.0
        w_ie_mean = float(np.abs(_ie).mean()) if _ie.size else 0.0
        spikes_in_mean = float(spikes[:, : self.st].mean()) if spikes.shape[0] else 0.0
        spikes_exc_mean = float(spikes[:, self.st : self.ex].mean()) if spikes.shape[0] else 0.0
        spikes_inh_mean = float(spikes[:, self.ex : self.ih].mean()) if spikes.shape[0] else 0.0

        # pre-trace distribution over ACTIVE (>0) neurons — the values x_tar is
        # compared against in trace-STDP. The marginal median is ~0 in the sparse
        # regime; the UPPER percentiles mark the driver range, so a usable static
        # x_tar sits around p75-p95 here (SE on inputs, EE on exc). These read off
        # the threshold directly instead of guessing it.
        qs = [50, 75, 90, 95, 99]
        se_all = spike_trace[: self.st]
        ee_all = spike_trace[self.st : self.ex]
        se_act = se_all[se_all > 0.0]
        ee_act = ee_all[ee_all > 0.0]
        se_q = np.percentile(se_act, qs) if se_act.size else np.zeros(len(qs))
        ee_q = np.percentile(ee_act, qs) if ee_act.size else np.zeros(len(qs))
        ...
        self._diag = {
            ...
            "se_trace_p75": float(se_q[1]),
            "se_trace_p90": float(se_q[2]),
            "se_trace_p99": float(se_q[4]),
            "ee_trace_p75": float(ee_q[1]),
            "ee_trace_p90": float(ee_q[2]),
            "ee_trace_p99": float(ee_q[4]),
            "w_se_mean": w_se_mean,
            "w_se_std": w_se_std,
            "w_ee_mean": w_ee_mean,
            "w_ee_std": w_ee_std,
            "w_ei_mean": w_ei_mean,
            "w_ie_mean": w_ie_mean,
            "spikes_in_mean": spikes_in_mean,
            "spikes_exc_mean": spikes_exc_mean,
            "spikes_inh_mean": spikes_inh_mean,
        }
        ...
        print("--- Diagnostics (pre-trace scale -> set static x_tar here) ---")
        print(
            f"SE input trace (active n={se_act.size}):  "
            f"p50={se_q[0]:.4f} p75={se_q[1]:.4f} p90={se_q[2]:.4f} p95={se_q[3]:.4f} p99={se_q[4]:.4f}"
        )
        print(
            f"EE exc   trace (active n={ee_act.size}):  "
            f"p50={ee_q[0]:.4f} p75={ee_q[1]:.4f} p90={ee_q[2]:.4f} p95={ee_q[3]:.4f} p99={ee_q[4]:.4f}"
        )
