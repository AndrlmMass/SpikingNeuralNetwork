...
def update_membrane_potential(
    ...
    print(W_exc[:-N_inhib_neurons].shape, S[:-N_inhib_neurons].shape)

    # Update I_in
    I_in_e = np.dot(W_exc[:-N_inhib_neurons].T, S[:-N_inhib_neurons])
    I_in_e_inh = np.dot(W_exc[-N_inhib_neurons:].T, S[-N_inhib_neurons:])
    print(I_in_e.shape)
    print(I_in_e_inh.shape)
