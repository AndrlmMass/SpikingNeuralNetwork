"""
TraceSTDP + WTA inhibition test.

Usage
-----
# Default run
python experiments/generic_testing/test_wta_trace.py

# Without WTA (Mexican-hat baseline)
python experiments/generic_testing/test_wta_trace.py --no-wta

# Quick smoke-test (~2 min)
python experiments/generic_testing/test_wta_trace.py --train-all 1000 --epochs 1
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="TraceSTDP + WTA inhibition test")

    p.add_argument("--no-wta", action="store_true",
                   help="Use Mexican-hat inhibition instead of WTA")
    ...
    p.add_argument("--peak-ei", type=float, default=2.0)
...
def main():
    ...
    label = "trace_wta" if use_wta else "trace_mexhat"
    ...
    print("\n" + "=" * 60)
    print(f"  TraceSTDP + {'WTA' if use_wta else 'Mexican-hat'} inhibition")
    print("=" * 60)
    print(f"  Dataset       : {args.dataset}")
    print(f"  Train images  : {args.train_all} / epoch  x {args.epochs} epochs")
    print(f"  WTA           : {use_wta}")
    print(f"  lr            : {args.lr}   tau_trace: {args.tau_trace}")
    ...
    print(f"\n{'=' * 55}")
    print(f"Rule           : TraceSTDP + {'WTA' if use_wta else 'Mexican-hat'}")
